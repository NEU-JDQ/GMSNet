...
from experiment_utils import classification_metrics, write_manifest, check_checkpoint_manifest
...
def parse_args():
    ...
    parser.add_argument('--tta', action='store_true', help='Opt in to horizontal-flip test-time augmentation')
    parser.add_argument('--hash-images', action='store_true')
    return parser.parse_args()


def run_test(model, loader, device, tta=False):
    model.eval()
    model = model.float()
    all_preds = []
    all_labels = []
    ...
    with torch.no_grad():
        ...
        for batch in loop:
            ...
            if tta:
                ...
                logits_flipped = model({'image': flipped_images, 'text_tokens': text_inputs})
...
def main():
    ...
    check_checkpoint_manifest(args.checkpoint_path, args, task_info, split_audit)
    ...
    true_labels, pred_labels = run_test(model, test_loader, device, tta=args.tta)

    metrics, class_report = classification_metrics(true_labels, pred_labels, label_map)
    report_str = f"Accuracy: {metrics['accuracy']:.4f}\nMacro F1: {metrics['macro_f1']:.4f}\n"
    report_str += f"Weighted F1: {metrics['weighted_f1']:.4f}\n"
    report_str += f"Classes: {num_classes}; fixed-label macro F1; horizontal-flip TTA: {args.tta}\n"
    ...
    write_manifest(Path(final_output_dir) / 'evaluation_config.json', args, task_info, split_audit,
                   checkpoint_sha256=sha256_file(args.checkpoint_path), metrics=metrics)
    ...
    df = pd.DataFrame({
        'Image Path': [row['image_path'] for row in test_set.data_list],
