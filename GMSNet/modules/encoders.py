...
class ConvNextVisualEncoder(nn.Module):
    def __init__(self, weights_path=str(Path(__file__).resolve().parents[2] / 'local_models/convnextv2_base/model.safetensors')):
        super().__init__()

        if weights_path is not None and not os.path.isfile(weights_path):
            raise FileNotFoundError(f'Visual pretrained weights not found: {weights_path}. '
                                    'Supply --visual_weights; training cannot silently start randomly.')
        ...
        if weights_path is not None:
            ...
            result = load_checkpoint(self.backbone, weights_path, strict=False)
            if result is not None and getattr(result, 'missing_keys', []):
                raise RuntimeError(f'Visual checkpoint has missing backbone keys: {result.missing_keys}')
        # weights_path=None is for immediate strict loading of a full trained model.
