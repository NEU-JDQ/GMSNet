...
class BaseFusionModule(nn.Module):
    ...
    def forward(self, visual_feats, text_feats, text_attention_mask=None):
...
class AttentionPooling(nn.Module):
    ...
    def forward(self, x, padding_mask=None):
        # Keep module indices/state-dict keys unchanged for existing checkpoints.
        scores = self.attention[:3](x)
...
class AGMFusion(BaseFusionModule):
    ...
    def forward(self, visual_feats, text_feats, text_attention_mask=None):
        ...
        if text_attention_mask is not None:
            if text_attention_mask.shape != text_feats.shape[:2]:
                raise ValueError('Text attention mask must match (batch, sequence)')
            padding_mask = text_attention_mask.to(text_feats.device).eq(0)
            if padding_mask.all(dim=1).any():
                raise ValueError('Each sample needs at least one valid text token')
        ...
        t_intra = self.txt_self_attn(t_embed, src_key_padding_mask=padding_mask)  

        # Apply bidirectional cross-attention to the unimodal feature sequences.
        v_fused = self.img2text_cross_attn(tgt=v_intra, memory=t_intra, memory_key_padding_mask=padding_mask)  

        t_fused = self.text2img_cross_attn(tgt=t_intra, memory=v_intra, tgt_key_padding_mask=padding_mask)  


        v_pool = self.vis_pooling(v_fused)  

        t_pool = self.txt_pooling(t_fused, padding_mask=padding_mask)  
